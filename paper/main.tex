...
\textbf{Sharpe Ratio}: annualised from the step-level reward signal $R_t$
(Eq.~\ref{eq:reward}), i.e., $\text{SR} = \nicefrac{\mu_R}{\sigma_R} \times \sqrt{8760}$.
Note this is \emph{not} a mark-to-market portfolio return Sharpe; it reflects
the ratio of mean fee-minus-LVR-minus-gas reward to its volatility, and
therefore takes values far above those typical of return-based Sharpe ratios.
...
\caption{ALVRO evaluation results on held-out split (4{,}799 hours).
         LLM column shows mean $\pm$ std over 3 independent runs; rule-based is a single run.}
\label{tab:results}
\begin{tabular}{lcc}
\toprule
Metric & Rule-based Sentinel & LLM Sentinel (DeepSeek) \\
\midrule
LP-to-HODL Ratio $\rho$ & 1.3235 & \textbf{1.357 $\pm$ 0.021} \\
Gas Efficiency & 2.80$\times$ & \textbf{(2.98 $\pm$ 0.09)$\times$} \\
Net Reward (\$) & +43{,}284 & \textbf{+46{,}885 $\pm$ 2{,}218} \\
Sharpe Ratio (ann.) & 210.2 & \textbf{213.7 $\pm$ 14.2} \\
...
\bottomrule
\multicolumn{3}{l}{\small Initial capital: \$100{,}000. Std computed over 3 runs (sample std, $n=3$).}
\end{tabular}
\end{table}
...
2.98$\times$ vs.\ 2.80$\times$.  Analysis of the 4{,}798-step evaluation trajectory shows the LLM sentinel
operates predominantly in the calm regime: $\lambda \in [0.75, 1.0]$ for
73.5\% of steps, shifting to a defensive posture ($\lambda > 1.25$) for
18.0\% of steps during elevated-volatility episodes.  The overall distribution
has mean $\lambda = 0.961$ and std $0.272$, ranging from 0.65 to 1.85,
confirming that the LLM actively discriminates between market regimes rather
than producing stochastic noise around a fixed value.
...
On 2.7 years of real WETH/USDC data, the DeepSeek LLM sentinel variant
achieves a \textbf{36\% outperformance} of a passive 50/50 HODL benchmark
(LP-to-HODL = 1.36) with a gas efficiency of 2.98$\times$ and a net reward
of \$46{,}885 on \$100{,}000 initial capital (averaged over 3 evaluation runs).
The LLM sentinel further improves upon the rule-based variant by 8.3\% in
net reward and 2.5\% in LP-to-HODL ratio, validating the benefit of
narrative-conditioned risk modulation.  These results establish that action
masking for gas-gate enforcement is effective, that the asymmetric range
formulation provides a useful defensive mechanism, and that LLM-generated
risk signals yield measurable improvements over rule-based alternatives.

\paragraph{Limitations and Broader Impact.}
The current evaluation covers a single WETH/USDC pool in simulation; real
on-chain deployment would introduce slippage, MEV exposure, and discrete tick
mechanics not modelled here.  Results from 3 LLM evaluation runs show moderate
variance (LP-to-HODL $\pm 0.021$), and ablation studies isolating each
innovation are deferred to future work.
On the societal side, widespread adoption of automated LP agents could
concentrate liquidity provision among well-resourced algorithmic actors,
potentially disadvantaging passive retail LPs who lack access to such tools
and reducing the diversity of on-chain market-making.

Future work will conduct ablation studies over the three innovations in
isolation, extend evaluation to multi-pool environments with shared policy
networks, and explore fine-tuned domain-specific LLMs as drop-in replacements
for the DeepSeek Sentinel.
...
Justification: Section~6 acknowledges that evaluation is conducted on a single WETH/USDC pool in simulation, that ablation studies over individual innovations are deferred to future work, and that multi-pool generalisation remains an open problem.
...
Justification: Section~5.1 describes the dataset, chronological 80/20 split, and feature engineering. Table~1 provides all hyperparameters. Section~4.6 details the MaskablePPO training setup.
...
Justification: Table~\ref{tab:results} reports mean $\pm$ sample standard deviation over $n=3$ independent evaluation runs for all primary LLM metrics: LP-to-HODL $1.357 \pm 0.021$, Gas Efficiency $(2.98 \pm 0.09)\times$, Net Reward $\$46{,}885 \pm \$2{,}218$, Sharpe $213.7 \pm 14.2$. Error bars capture run-to-run variability from different random seeds. The rule-based sentinel is deterministic and thus requires only a single evaluation run.
...
Justification: Training uses a standard MacBook CPU (Apple M-series) for 1M timesteps, taking approximately 42 minutes for the rule-based sentinel ($\approx$400 steps/sec). The LLM sentinel requires an internet connection and a DeepSeek API key; throughput is API-bound at approximately 10 steps/sec, yielding a training time of approximately 26 hours for 1M steps.
...
Justification: The introduction discusses the positive impact of improving LP returns and DeFi accessibility for retail participants. The conclusion explicitly notes a negative societal impact: algorithmic LP agents could concentrate market-making power among well-resourced actors, potentially reducing liquidity diversity and disadvantaging passive retail LPs who cannot run automated systems. No mechanisms for misuse as a weapon or for generating disinformation exist in this domain-specific financial agent.
